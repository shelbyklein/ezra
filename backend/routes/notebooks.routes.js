...
router.get('/recent', auth_middleware_1.default, async (req, res) => {
    try {
        ...
        const pageCounts = await (0, db_1.default)('notebook_pages')
            .whereIn('notebook_id', notebookIds)
            .groupBy('notebook_id')
            .select('notebook_id')
            .count('id as count');
        const pageCountMap = pageCounts.reduce((acc, pc) => {
            acc[pc.notebook_id] = parseInt(pc.count);
            return acc;
        }, {});
...
router.get('/starred-pages', auth_middleware_1.default, async (req, res) => {
    try {
        console.log('Fetching starred pages for user:', req.user.userId);
        console.log('Database object exists:', !!db_1.default);
        const starredPages = await (0, db_1.default)('notebook_pages as p')
            .join('notebooks as n', 'p.notebook_id', 'n.id')
            .where({
            'n.user_id': req.user.userId,
            'p.is_starred': 1 // SQLite uses 1/0 for boolean
        })
            .select('p.id', 'p.title', 'p.slug', 'p.updated_at', 'p.notebook_id', 'n.title as notebook_title', 'n.icon as notebook_icon')
...
router.get('/', auth_middleware_1.default, async (req, res) => {
    try {
        const notebooks = await (0, db_1.default)('notebooks')
            ...
            .select('notebooks.*', 'projects.title as project_name', // Database uses 'title' not 'name'
