...
router.get('/project/:projectId', auth_middleware_1.default, async (req, res) => {
    try {
        ...
        let taskTags = [];
        if (taskIds.length > 0) {
            taskTags = await (0, db_1.default)('task_tags as tt')
...
router.post('/', auth_middleware_1.default, async (req, res) => {
    try {
        ...
        await (0, db_1.default)('tasks').insert(task);
...
router.post('/reorder', auth_middleware_1.default, async (req, res) => {
    try {
...
router.post('/natural-language', auth_middleware_1.default, async (req, res) => {
    try {
        ...
        const currentTasks = await (0, db_1.default)('tasks')
            .where({ project_id: projectId })
            .select('id', 'title', 'status');
        // Parse the natural language command
        const parsedCommand = await (0, anthropic_1.parseNaturalLanguageCommand)(req.user.userId, command, { projectId, currentTasks });
        if (!parsedCommand) {
        ...
        switch (parsedCommand.action) {
            ...
            case 'move':
                ...
                const tasksToUpdate = await (0, db_1.default)('tasks')
            ...
            case 'delete':
                ...
                // Verify user owns all tasks
                const tasksToDelete = await (0, db_1.default)('tasks')
                    .whereIn('id', parsedCommand.targetTasks.taskIds)
                    .where({ project_id: projectId });
            ...
            case 'bulk':
                ...
                let query = (0, db_1.default)('tasks').where({ project_id: projectId });
