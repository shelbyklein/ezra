    ...
    try {
        ...
        if (searchTypes.includes('task')) {
            const tasks = await (0, db_1.default)('tasks')
                ...
                .select('tasks.id', 'tasks.title', 'tasks.description', 'tasks.project_id as projectId', 'projects.name as projectName');
            tasks.forEach(task => {
        ...
        if (searchTypes.includes('page')) {
            const pages = await (0, db_1.default)('notebook_pages')
                ...
                .select('notebook_pages.id', 'notebook_pages.title', 'notebook_pages.content', 'notebook_pages.notebook_id as notebookId', 'notebooks.title as notebookTitle');
            pages.forEach(page => {
