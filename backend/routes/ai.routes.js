...
const contextSearch_1 = require("../utils/contextSearch");
...
router.post('/tasks/:id/enhance', auth_middleware_1.default, async (req, res) => {
    try {
        const taskId = parseInt(req.params.id);
        const userId = req.user.userId;
        // Check if user has API key
        const client = await (0, anthropic_1.createAnthropicClient)(userId);
        if (!client) {
        ...
        // Enhance the task
        try {
            const enhancement = await (0, anthropic_1.enhanceTaskWithAI)(userId, task.title, task.description);
            if (!enhancement) {
...
router.post('/enhance', auth_middleware_1.default, async (req, res) => {
    try {
        ...
        // Enhance the task
        try {
            const enhancement = await (0, anthropic_1.enhanceTaskWithAI)(userId, title, description);
            if (!enhancement) {
                return res.status(500).json({ error: 'Failed to generate enhancement' });
            }
...
router.post('/suggest-tasks', auth_middleware_1.default, async (req, res) => {
    try {
        ...
        const existingTasks = await (0, db_1.default)('tasks')
...
router.post('/search-context', auth_middleware_1.default, async (req, res) => {
    try {
        ...
        const results = await (0, contextSearch_1.searchUserContent)(query, userId, options);
        res.json({
            results,
            formattedContext: (0, contextSearch_1.formatContextForAI)(results),
            citations: (0, contextSearch_1.generateSourceCitations)(results)
        });
    }
    catch (error) {
...
router.post('/chat', auth_middleware_1.default, async (req, res) => {
    try {
        ...
        try {
            ...
            searchResults = await (0, contextSearch_1.searchUserContent)(message, userId, { limit: 10 });
            if (searchResults.length > 0) {
                searchContext = (0, contextSearch_1.formatContextForAI)(searchResults);
                citations = (0, contextSearch_1.generateSourceCitations)(searchResults);
            }
        }
        catch (searchError) {
        ...
        const projectContext = context?.currentProjectId ?
            await (0, db_1.default)('projects').where({ id: context.currentProjectId, user_id: userId }).first() : null;
        const recentTasks = context?.currentProjectId ?
            await (0, db_1.default)('tasks').where({ project_id: context.currentProjectId }).limit(10).select('id', 'title', 'status') : [];
        const userProjects = await (0, db_1.default)('projects').where({ user_id: userId }).select('id', 'name');
        // Get notebook context if available
        const notebookContext = context?.currentNotebookId ?
            await (0, db_1.default)('notebooks').where({ id: context.currentNotebookId, user_id: userId }).first() : null;
        const pageContext = context?.currentPageId ?
            await (0, db_1.default)('notebook_pages as p')
...
function parseMarkdownToTipTap(markdown, highlight = false) {
...
async function executeAction(action, parameters, userId, context) {
    try {
        switch (action) {
            case 'create_project':
                const newProject = await (0, db_1.default)('projects').insert({
                    user_id: userId,
            ...
            case 'create_task':
                ...
                const projectCheck = await (0, db_1.default)('projects')
                    .where({
                    id: projectId,
                    user_id: userId
                })
                ...
                const maxPosition = await (0, db_1.default)('tasks')
                    .where({
                ...
                await (0, db_1.default)('tasks').insert(newTask);
                const createdTask = await (0, db_1.default)('tasks')
                    .where({ project_id: projectId })
                    .orderBy('id', 'desc')
                    .first();
                return {
            ...
            case 'create_multiple_tasks':
                ...
                // Verify user owns the project
                const batchProjectCheck = await (0, db_1.default)('projects')
                    .where({
                    id: batchProjectId,
                    user_id: userId
                })
                ...
                for (const task of parameters.tasks) {
                    ...
                    const maxPos = await (0, db_1.default)('tasks')
                        .where({
                    ...
                    await (0, db_1.default)('tasks').insert(newBatchTask);
                    const created = await (0, db_1.default)('tasks')
            ...
            case 'delete_task':
                ...
                const tasksToDelete = await (0, db_1.default)('tasks as t')
            ...
            case 'update_page':
                ...
                const pageCheck = await (0, db_1.default)('notebook_pages as p')
                    .join('notebooks as n', 'p.notebook_id', 'n.id')
                    .where({
            ...
            case 'create_page':
                ...
                const notebookCheck = await (0, db_1.default)('notebooks')
                    .where({
                    id: notebookId,
                    user_id: userId
                })
                ...
                const slug = parameters.title.toLowerCase()
                    ...
                    .replace(/^-|-$/g, '');
                // Get max position
                const maxPos = await (0, db_1.default)('notebook_pages')
                ...
                await (0, db_1.default)('notebook_pages').insert(newPage);
